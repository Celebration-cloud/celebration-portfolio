...
export const Navbar = () => {
  ...
  return (
    <HeroUINavbar shouldHideOnScroll maxWidth="2xl" position="sticky">
      ...
      <NavbarContent
        className="hidden sm:flex basis-3/5 sm:basis-full navbar"
        justify="center"
      >
        <ul
          className="hidden lg:flex gap-10 justify-start ml-2 nav nav-pills"
          id="navbar-example2"
        >
      ...
      <NavbarContent
        className="lg:hidden basis-1/5 sm:basis-full"
        justify="end"
      >
        <NavbarMenuToggle />
      </NavbarContent>
        ...
        <div className="mx-4 mt-2 flex flex-col gap-2">
          {siteConfig.navItems.map((item, index) => (
            <NavbarMenuItem key={`${item}-${index}`} className="nav-item">
              <Link color={"foreground"} className="nav-link" href={item.href} size="lg">
                {item.label}
              </Link>
            </NavbarMenuItem>
          ))}
